...
export function getLocalizedText(text: MultilingualText | string | undefined, preferredLang = "fi"): string {
  if (!text) return ""
...
export async function fetchPinkkaGroupWithStacks(groupId: number): Promise<PinkkaGroup | null> {
  try {
    const response = await fetch(`${BASE_URL}/pinkkas/${groupId}`, {
      cache: "no-store",
    })
    if (!response.ok) {
      throw new Error(`Failed to fetch group ${groupId}: ${response.statusText}`)
    }
    return await response.json()
  } catch (error) {
...
export async function fetchPinkkaSubStack(subStackId: number): Promise<PinkkaSubStack | null> {
  try {
    const response = await fetch(`${BASE_URL}/subpinkkas/${subStackId}`, {
      cache: "no-store",
    })
    if (!response.ok) {
      throw new Error(`Failed to fetch substack ${subStackId}: ${response.statusText}`)
    }
    return await response.json()
  } catch (error) {
...
export async function fetchPinkkaSpecies(speciesId: number): Promise<PinkkaSpeciesDetail | null> {
  try {
    const response = await fetch(`${BASE_URL}/speciescards/${speciesId}`, {
      cache: "no-store",
    })
    if (!response.ok) {
      throw new Error(`Failed to fetch species ${speciesId}: ${response.statusText}`)
    }
    return await response.json()
  } catch (error) {
