...
export async function getGroups(userId?: string): Promise<Group[]> {
  ...
  if (userId) {
    q = query(collection(db, "groups"), where("createdBy", "==", userId), orderBy("order"))
  }
...
export async function deleteGroup(groupId: string): Promise<void> {
  ...
  const stacksQuery = query(collection(db, "stacks"), where("groupId", "==", groupId))
...
// Stack operations
export async function createStack(stack: Omit<Stack, "id" | "createdAt" | "updatedAt">): Promise<string> {
...
export async function getStacks(groupId?: string, userId?: string): Promise<Stack[]> {
  ...
  if (groupId && userId) {
    ...
    q = query(collection(db, "stacks"), where("groupId", "==", groupId), orderBy("order"))
  } else if (userId) {
    q = query(collection(db, "stacks"), where("createdBy", "==", userId), orderBy("order"))
  }
...
export async function updateStack(stackId: string, updates: Partial<Stack>): Promise<void> {
...
export async function deleteStack(stackId: string): Promise<void> {
  ...
  const speciesQuery = query(collection(db, "species"), where("stackId", "==", stackId))
...
// Species operations
export async function createSpecies(species: Omit<Species, "id" | "createdAt" | "updatedAt">): Promise<string> {
...
export async function getSpecies(stackId?: string): Promise<Species[]> {
  ...
  if (stackId) {
    q = query(collection(db, "species"), where("stackId", "==", stackId), orderBy("order"))
  }
...
export async function getSpeciesById(speciesId: string): Promise<Species | null> {
...
export async function updateSpecies(speciesId: string, updates: Partial<Species>): Promise<void> {
...
// Image operations
export async function uploadSpeciesImage(speciesId: string, file: File, order: number): Promise<SpeciesImage> {
...
export async function deleteSpeciesImage(speciesId: string, imageUrl: string): Promise<void> {
  try {
    ...
    if (speciesDoc.exists()) {
      const images = speciesDoc.data().images || []
      const updatedImages = images.filter((img: SpeciesImage) => img.url !== imageUrl)
      await updateDoc(doc(db, "species", speciesId), {
...
// Batch reordering
export async function reorderItems(collectionName: string, items: { id: string; order: number }[]): Promise<void> {
