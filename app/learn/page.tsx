...
export default function LearnPage() {
  ...
  const [stacksByGroup, setStacksByGroup] = useState<{ [key: string]: Stack[] }>({})
  ...
  const loadData = async () => {
    try {
      ...
      for (const group of groupsData) {
        stacksData[group.id] = await getStacks(group.id)
      }
    ...
    <div className="min-h-screen bg-gradient-to-b from-background to-secondary/20">
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          <p className="text-muted-foreground">Choose a stack to study with flashcards or take a quiz</p>
        </div>
                ...
                {(stacksByGroup[group.id] || []).map((stack) => (
                  <Card key={stack.id} className="hover:shadow-lg transition-shadow">
                    <CardHeader>
                      ...
                      {stack.description && <CardDescription>{stack.description}</CardDescription>}
                    </CardHeader>
          ...
          {groups.length === 0 && (
            <Card>
              <CardContent className="py-12 text-center text-muted-foreground">
                ...
                <p className="text-sm">Check back later or contact an editor to add content</p>
              </CardContent>
            </Card>
          )}
        </div>
