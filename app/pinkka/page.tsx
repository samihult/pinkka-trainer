...
export default function PinkkaPage() {
  ...
  const [subStacksByGroup, setSubStacksByGroup] = useState<{ [key: number]: PinkkaSubStack[] }>({})
    ...
    try {
      ...
      for (const group of groupsData) {
        ...
        if (groupDetail?.subPinkkas) {
          subStacksData[group.id] = groupDetail.subPinkkas.sort((a, b) => a.orderNo - b.orderNo)
        }
      }
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="mb-8 flex items-start justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-muted-foreground">Browse species collections from pinkka.laji.fi</p>
          </div>
          <div className="flex gap-2 flex-wrap">
            ...
            <Button asChild variant="outline">
              <a href="https://pinkka.laji.fi" target="_blank" rel="noopener noreferrer">
        ...
        {error && (
          <Card className="mb-6 border-destructive">
            <CardContent className="py-6">
              ...
              <Button onClick={loadData} className="mt-4 bg-transparent" variant="outline">
                Try Again
              </Button>
            </CardContent>
          </Card>
        ...
        <div className="space-y-8">
          {groups.map((group) => {
            ...
            return (
              <div key={group.id}>
                <h2 className="text-2xl font-bold mb-4">{groupName || `Group ${group.id}`}</h2>
                {groupDesc && <p className="text-muted-foreground mb-4">{groupDesc}</p>}

                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {subStacks.map((subStack) => {
                    const subStackName = getLocalizedText(subStack.name, language)
                    const subStackDesc = getLocalizedText(subStack.description, language)
                    const speciesCount = subStack.speciesCards?.length || 0

                    return (
                      <Card key={subStack.id} className="hover:shadow-lg transition-shadow">
                        <CardHeader>
                          <CardTitle className="flex items-center gap-2 text-balance">
                            <BookOpen className="h-5 w-5 text-primary flex-shrink-0" />
                            <span>{subStackName || `Stack ${subStack.id}`}</span>
                          </CardTitle>
                          {subStackDesc && <CardDescription className="text-pretty">{subStackDesc}</CardDescription>}
                          {speciesCount > 0 && <p className="text-sm text-muted-foreground">{speciesCount} species</p>}
                        </CardHeader>
          ...
          {groups.length === 0 && !error && (
            <Card>
              <CardContent className="py-12 text-center text-muted-foreground">
                ...
                <p className="text-sm">The external API may be temporarily unavailable</p>
              </CardContent>
            </Card>
          )}
        </div>
